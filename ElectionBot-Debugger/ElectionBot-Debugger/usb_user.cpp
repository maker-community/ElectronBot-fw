...
//#define MY_VID 1155    // 0x0483
//#define MY_PID 22336   //0x5740

//unsigned short MY_VID = 1155 ;
//unsigned short MY_PID = 22336;
unsigned short MY_VID = 4097 ;
unsigned short MY_PID = 32803;
...
int USB_Read_Process(char *dat, int lenth)
{
    ...
    ret = transfer_bulk_async(dev, EP_IN, dat, lenth, 50);
#else
    // Running a sync read test
    ret = usb_bulk_read(dev, EP_IN, dat, lenth, 50);
#endif
    //    if (ret < 0)
    //    {
...
*/
int transfer_bulk_async(usb_dev_handle *dev,
                               int ep,
                               char *bytes,
                               int size,
                               int timeout)
...
{
   int ret =transfer_bulk_async(dev,EP_OUT,dat,lenth,timeout);
   return ret;
}
