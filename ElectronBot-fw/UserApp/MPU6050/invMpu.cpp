...
static int get_accel_prod_shift(float *st_shift)
{
    ...
    if (MPU_Read_Len(st.hw->addr, 0x0D, 4, tmp))
        return 0x07;
                  ...
                  unsigned char *data)
{
    ...
    MPU_Write_Byte(st.reg->bank_sel,tmp[0]);
    MPU_Write_Byte(st.reg->bank_sel+1,tmp[1]);
     //   read_tmp[0]=MPU_Read_Byte(st.reg->bank_sel);
    //    read_tmp[1]=MPU_Read_Byte(st.reg->bank_sel+1);
    i2c_read(st.hw->addr, st.reg->bank_sel, 2, read_tmp);
    myDebug("write mem tmp[0]=0x%02d tmp[1]=0x%02d\r\nread_tmp[0]=0x%02d read_tmp[1]=0x%02d\r\n ",tmp[0],tmp[1],read_tmp[0],read_tmp[1]);
    //HAL_Delay(10);
    while((read_tmp[1]-1)!=tmp[1])
    {
        MX_I2C1_Init();
    ...
    if (i2c_write(st.hw->addr, st.reg->mem_r_w, length, data))
        return -1;
    return 0;    MPU_Write_Byte(st.reg->bank_sel,tmp[0]);
    MPU_Write_Byte(st.reg->bank_sel+1,tmp[1]);
}
                  ...
                  unsigned char *data)
{
    ...
    {
        MPU_Write_Byte(st.reg->mem_r_w+i,data[i]);
        HAL_Delay(1);
    }
    //if (i2c_write(st.hw->addr, st.reg->mem_r_w, length, data))
    //    return -1;
                 ...
                 unsigned char *data)
{
    ...
    MPU_Write_Byte(st.reg->bank_sel+1,tmp[1]);
    i2c_read(st.hw->addr, st.reg->bank_sel, 2, read_tmp);
    myDebug("read mem tmp[0]=0x%02d tmp[1]=0x%02d\r\nread_tmp[0]=0x%02d read_tmp[1]=0x%02d\r\n ",tmp[0],tmp[1],read_tmp[0],read_tmp[1]);
   // HAL_Delay(10);
    while((read_tmp[1]-1)!=tmp[1])
    {
        MX_I2C1_Init();
                 ...
                 unsigned char *data)
{
   ...
   //     return -1;
    ...
    for(int i=0;i<length;i++)
    {
        data[i]=MPU_Read_Byte(st.reg->mem_r_w+i);
        HAL_Delay(1);
    }
    //if (i2c_read(st.hw->addr, st.reg->mem_r_w, length, data))
    //    return -1;
...
//#define LOAD_CHUNK  (2)
    ...
    for (ii = 0; ii < length; ii += this_write)
    {
        ...
        if (mpu_read_mem(ii, this_write, cur))
            res=4;
            //return 4;
        //if (memcmp(firmware+ii, cur, this_write)) //by xxm 20220921
        //    return -2;
        for(unsigned short i=0;i<this_write;i++)
        {
            if (firmware[ii+i]==cur[i])
            {
        ...
        for(unsigned short j=0;j<this_write;j++)
        {
            myDebug("firmware[%d]=0x%x,cur[%d]=0x%x\r\n",ii+j,firmware[ii+j],j,cur[j]);
            //HAL_Delay(10);
        }
        if(printf_flag_en==true)
        //if(1)
        {
           // printf_flag_en= false;
            reTry=3;
            while(printf_flag_en==true && reTry>0)
            {
                printf_flag_en=false;
